//Saksham Singhal IIIT Hyderabad
...
struct laser
{
	float x;float y;float thet;int state;int hit_state;float nx,ny;
};
...
int power_up=0;
int speed_time=0;float spider_speed=0.1f;
...
float tri_x = 2.0f;
float tri_y = -19.0f;
float theta = 0.0f; char present='c';
vector <struct spider> spiders;  //Details about all the spiders
vector <struct spider> ::iterator it;
vector <struct spider> ::iterator ix;
struct spider spidy;
...
void drawScene()
{
	...
	// Draw Box
	glTranslatef(2.0f, 0.0f, -28.0f);
	glColor3f(0.0f, 0.0f, 0.0f);
	drawBox(box_len);

	//Draw the BASE
	glPushMatrix();
	glTranslatef(-20.0f,-22.0f,-28.0f);
	...
	glTranslatef(red_x,red_y, -28.0f);
	drawsquare();
	glPopMatrix();
	...
	glTranslatef(green_x,green_y ,-28.0f);
	...
	// Draw Cannon
	glPushMatrix();
	glTranslatef(tri_x, tri_y, -28.0f);
	...
	drawTriangle();
	glPopMatrix();
	for(it=spiders.begin();it!=spiders.end();++it)
	{
		if(it->state!=2)
		{
			glPushMatrix();
			...
			{
				glColor3f(1.0f,0.0f,0.0f);
				glTranslatef(it->sp_x,it->sp_y, -28.0f);
				drawsquare1();
			}
			else if(it->sp_col=='b')
			{
			...
			else if(it->sp_col=='y')   //POWER UP SPIDER
			{
	...
	for(il=lasers.begin();il!=lasers.end();++il)
	{
		if(il->state!=2)
		{
			...
			if(il->hit_state==0)
			{
				glTranslatef(il->x-(sin(DEG2RAD(il->thet))*2.5f),il->y+(cos(DEG2RAD(il->thet))*2.5f),-28.0f);
			...
			{
				glTranslatef(il->nx,il->ny,-28.0f);
				glRotatef(il->thet,0.0f,0.0f,1.0f);
				glBegin(GL_LINES);
				glVertex2f(0.0f,0.0f);
	...
	glTranslatef(-38.0f,16.0f,-28.0f);
	glBegin(GL_QUADS);
	glVertex2f(-2,-2);
	glVertex2f(-2, 3);
	glVertex2f(10, 3);
	...
	glPushMatrix();
	glColor3f(0.0f,0.0f,0.0f);
	glRasterPos3f(-38.0f,16.0f,-28.0f);
	glScalef(5.0f,5.0f,5.0f);
	char b[]={"My Score"};
	char new_sc[1000];
...
void update(int value) 
{
	if(!paused)
	{
		...
		if(exit_flag==1)
			exit_time+=10;
		/////////////////////////////////////////////////SPIDERS COLLISION WITH CANON AND BOXES/////////////////////////////////////////////////////
			...
			if(it->state==0 and it->sp_y<=-18.0f)
			{
				if(((it->sp_x>=tri_x-2.0f and it->sp_x<=tri_x+1.9f) or (it->sp_x+4.0f>=tri_x-1.9f and it->sp_x+4.0f<=tri_x+2.0f)))
				{
					...
					if(it->sp_col=='y')
					{
						...
						for(ix=spiders.begin();ix!=spiders.end();++ix)
							ix->state=2;
					}
					else
					{
		...
		for(il=lasers.begin();il!=lasers.end();++il)
		{
			if(il->state!=2)
			{
				if(il->hit_state==0)
				{
					if((il->x-sin(DEG2RAD(il->thet))*1.0f)>=box_len+1.0f || (il->x-sin(DEG2RAD(il->thet)))*1.0f<=-19.0f)
					{
...
void generate(int val)
{
	if(!paused)
	{
		spidy.sp_x=((rand()%6)*4.0f);
		spidy.sp_y=21.0f;spidy.state=0;
		int sign=rand()%2;
		if(sign==1)
			spidy.sp_x*=-1;
		int c=rand()%4;
...
void drawsquare1()
{
	...
	glEnd();
	glBegin(GL_QUADS);
	glVertex3f(1.33f, 3.5f, 0.0f);
	glVertex3f(2.67f, 3.5f, 0.0f);
	glVertex3f(2.67f, 3.0f, 0.0f);
	glVertex3f(1.33f, 3.0f, 0.0f);
	glEnd();
	glBegin(GL_LINES);
	glVertex3f(3.0f,3.0f,0.0f);
	...
	glVertex3f(0.5f,3.5f,0.0f);
	glVertex3f(3.0f,2.33f,0.0f);
	glVertex3f(3.5f,2.33f,0.0f);
	glVertex3f(4.0f,2.67f,0.0f);
	glVertex3f(3.5f,2.33f,0.0f);
	glVertex3f(3.0f,1.66f,0.0f);
	glVertex3f(3.5f,1.66f,0.0f);
	glVertex3f(4.0f,1.33f,0.0f);
	glVertex3f(3.5f,1.66f,0.0f);

	glVertex3f(1.0f,2.33f,0.0f);
	glVertex3f(0.5f,2.33f,0.0f);
	glVertex3f(0.0f,2.67f,0.0f);
	glVertex3f(0.5f,2.33f,0.0f);
	glVertex3f(1.0f,1.66f,0.0f);
	glVertex3f(0.5f,1.66f,0.0f);
	glVertex3f(0.0f,1.33f,0.0f);
	glVertex3f(0.5f,1.66f,0.0f);
...
void handleKeypress2(int key, int x, int y) 
{
	if(!paused)
	{
		...
		{
			if(present=='c'&& tri_x-2.0f>start&& tri_x<=end)
				tri_x -= 1.0;
		...
		else if(key==GLUT_KEY_DOWN && theta>-50)
			theta-=10;
		else if(key==GLUT_KEY_UP && theta<50)
			theta+=10;
	}
...
void handlemouse_drag(int x,int y)
{
	if(!paused)
	{
		...
		if(click==1)
		{
			if(present=='c' &&  mouseposx>start+2 && mouseposx<end)
			{
				tri_x=mouseposx;
			}
